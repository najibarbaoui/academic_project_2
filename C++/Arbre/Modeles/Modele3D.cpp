...
Modele3D::~Modele3D()
{
	...
	if (identificateursTextures_ != NULL)
...
void Modele3D::charger(const string& nomFichier)
{
	...
	this->calculerBoiteEnglobante(coinMin_, coinMax_);
	
	/// Calcule du rayon de la sphere englobante
	this->rayon_ = this->calculerSphereEnglobante();
}
...
void Modele3D::chargerTextures()
{
	...
	// Nombre de textures
	unsigned int nombreTextures = (unsigned int)mapTextures_.size();
...
double Modele3D::obtenirSphereEnglobante() const
{
	return rayon_;
}
...
void Modele3D::obtenirBoiteEnglobante(Vecteur3& coinMin, Vecteur3& coinMax) const
{
	coinMin = coinMin_;
	coinMax = coinMax_;
}
