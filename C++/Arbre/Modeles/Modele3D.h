...
public:
	...
	double obtenirSphereEnglobante() const;

	/// Calcule le cylindre englobant
	void calculerCylindreEnglobant(double& rayon, double& bas, double& haut) const;

	/// Obtient la boite englobante à partir du calcul déjà fait dans le constructeur
	void obtenirBoiteEnglobante(Vecteur3& coinMin, Vecteur3& coinMax) const;
